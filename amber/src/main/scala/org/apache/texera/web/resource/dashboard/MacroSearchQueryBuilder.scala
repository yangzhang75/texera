...
/**
  * Search builder for the unified "Macros" tab. Macros are stored in the
  * workflow table with kind = MACRO (a reusable, encapsulated sub-workflow),
  * so this mirrors WorkflowSearchQueryBuilder but selects ONLY kind = MACRO
  * rows. Entries are emitted as workflow-shaped DashboardClickableFileEntry so
  * the existing dashboard grid renders them with no frontend changes; clicking
  * a macro opens it (WorkflowResource 404s a MACRO row, and the workspace then
  * redirects into the macro drill-down editor).
  */
object MacroSearchQueryBuilder extends SearchQueryBuilder {
  ...
  override protected def constructFromClause(
    ...
    // Only macro definitions (kind = MACRO) belong in the Macros tab.
    condition = condition.and(WORKFLOW.KIND.eq(WorkflowKindEnum.MACRO))
