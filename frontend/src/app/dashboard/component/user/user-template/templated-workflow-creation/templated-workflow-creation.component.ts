...
export class TemplatedWorkflowCreationComponent implements AfterViewInit {
  ...
  // SUBMIT is disabled only while the workflow genuinely can't accept one (still loading, or
  // running). It is intentionally NOT greyed on "no changes": detecting "no changes" reliably
  // fought nz-input-number (which only commits a typed value on blur), so a freshly-typed number
  // kept the button looking inactive. Keeping it always-clickable when ready guarantees the hard
  // rule "any edit can be submitted". A click with nothing to apply is a silent no-op.
