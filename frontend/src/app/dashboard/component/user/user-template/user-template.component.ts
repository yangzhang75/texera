...
export class UserTemplateComponent implements OnInit, AfterViewInit {
  ...
  public onClickCreateNewTemplateFromDashboard(): void {
    ...
    this.templateService
      ...
      .subscribe({
        ...
        error: () => this.notificationService.error("Template creation failed"),
      });
  }
  ...
  public async onClickDuplicateTemplate(entry: DashboardEntry): Promise<void> {
    if (entry.template.template.tid) {
      try {
        ...
        this.notificationService.error("Error duplicating template");
      }
    }
  }
  ...
  public onClickOpenDownloadZip(): void {
    ...
    this.downloadService
      ...
      .subscribe({
        next: () => {},
        error: (err: unknown) => this.notificationService.error("Error downloading templates"),
      });
  }
    ...
    if (targetTids.length > 0) {
      this.templateService
        ...
        .subscribe({
          ...
          error: (err: unknown) => this.notificationService.error("Error duplicating templates"),
        });
    }
  }
    ...
    if (targetTids.length > 0) {
      this.templateService
        ...
        .subscribe({
          ...
          error: (err: unknown) => this.notificationService.error("Error deleting templates"),
        });
    }
  }
