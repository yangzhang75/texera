  ...
  setFormlyFormBinding(schema: CustomJSONSchema7) {
    ...
    const jsonSchemaMapIntercept = (
      ...
      if (
        ...
        // Don't offer the "configurable" checkbox on macro structural nodes:
        //  - MacroInput/MacroOutput boundary markers are stripped on generate,
        //    so marking them does nothing (empty Generate form);
        //  - a nested Macro node's own props (macroId/linkMode/...) aren't
        //    user-fillable params, and its inner params are out of scope here.
        // Only real operators expose configurable properties.
        const isMacroStructural =
          operator.operatorType === "MacroInput" ||
          operator.operatorType === "MacroOutput" ||
          operator.operatorType === "Macro";

        ConfigurablePropertyWrapperComponent.setupFieldConfig(
          mappedField,
          this.isTemplateMode() && !isMacroStructural,
      ...
      // Snapshot-only MVP: a macro REFERENCE is read-only except Display Name.
      // Link Mode (SNAPSHOT/LIVE) is cut — every macro is a frozen snapshot — so
      // that field is hidden along with the binding fields (macroId /
      // macroVersion / port counts / embedded snapshot / fusion), which are
      // managed by insert; hand-editing them corrupts the reference (mismatched
      // version/ports -> broken expansion). Hiding only gates the FORM control;
      // resetOnHide is false so model values are preserved and programmatic
      // setOperatorProperty (insert-time writes) is unaffected.
      if (
        this.currentOperatorSchema?.operatorType === "Macro" &&
        typeof mappedField.key === "string" &&
        !["displayName"].includes(mappedField.key)
      ) {
