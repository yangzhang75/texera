...
export class PropertyEditorComponent implements OnInit, OnDestroy, OnChanges {
  ...
  /**
   * Whether this panel owns the canvas panel's saved size/position. The Form View mounts
   * this same component inline in a preview box; only the docked canvas panel persists, so
  ...
  /** The choose-what-to-expose affordance appears only where the feature flag is on and the
   *  author has turned the Form View on for this workflow. */
  public get offersParameterizedCanvas(): boolean {
    return (
      this.config.env.formViewEnabled && this.workflowActionService.getWorkflowMetadata()?.isParameterized === true
    );
  }
  ...
  ngOnInit(): void {
    ...
    this.parameterizationService.choosing$.pipe(distinctUntilChanged(), untilDestroyed(this)).subscribe(choosing => {
