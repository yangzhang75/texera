...
export class WorkspaceComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  context$ = combineLatest([this.route.paramMap, this.route.queryParamMap]).pipe(
  ...
  loadTemplateWithId(tid: number): void {
    ...
    this.templateService
      ...
      .subscribe(
        (template: Template) => {
          const workspaceWorkflow = this.createWorkflowFromTemplate(template);
  ...
  // --- LIVE reference: open-time update prompt (never silent) ---------------
  // On opening a workflow, any LIVE macro node whose pinned macroVersion is
  // behind the definition's latest version prompts the user; only on confirm do
  // we re-embed the latest body + bump the pinned version. Cancel keeps the old
  // body. Scans the loaded CONTENT (sync + reliable, unlike the async graph);
  // the confirm-time mutation runs against the graph, ready by the time the user
  // clicks. Skipped in the embedded preview.
  private checkLiveMacroUpdates(workflow: Workflow): void {
    if (this.isEmbedded) return;
    const ops = workflow.content?.operators ?? [];
    const liveNodes = ops
      .filter(o => o.operatorType === "Macro" && o.operatorProperties?.["linkMode"] === "LIVE")
      .map(o => ({
        operatorID: o.operatorID,
        macroId: String(o.operatorProperties?.["macroId"] ?? ""),
        pinned: Number(o.operatorProperties?.["macroVersion"] ?? 0),
        name: String(o.operatorProperties?.["displayName"] ?? o.operatorProperties?.["macroId"] ?? "macro"),
      }))
      .filter(n => n.macroId !== "");
    if (liveNodes.length === 0) return;
    const wids = [...new Set(liveNodes.map(n => Number(n.macroId)).filter(w => Number.isFinite(w)))];
    this.macroService
      .getLatestVersions(wids)
      .pipe(untilDestroyed(this))
      .subscribe({
        next: latest => {
          const stale = liveNodes.filter(n => (latest[n.macroId] ?? n.pinned) > n.pinned);
          this.promptLiveUpdatesSequentially(stale, latest);
        },
        error: () => {
          /* couldn't reach the version endpoint -- silently skip (no prompt) */
        },
      });
  }

  private promptLiveUpdatesSequentially(
    stale: Array<{ operatorID: string; macroId: string; pinned: number; name: string }>,
    latest: Record<string, number>
  ): void {
    if (stale.length === 0) return;
    const [node, ...rest] = stale;
    this.nzModalService.confirm({
      nzTitle: "Macro update available",
      nzContent: `${node.name}: pinned v${node.pinned}, latest v${latest[node.macroId]} — update this reference?`,
      nzOkText: "Update",
      nzCancelText: "Keep current",
      nzOnOk: () => this.applyLiveUpdate(node, latest[node.macroId], rest, latest),
      nzOnCancel: () => this.promptLiveUpdatesSequentially(rest, latest),
    });
  }

  private applyLiveUpdate(
    node: { operatorID: string; macroId: string; pinned: number; name: string },
    latestVersion: number,
    rest: Array<{ operatorID: string; macroId: string; pinned: number; name: string }>,
    latest: Record<string, number>
  ): void {
    const continueNext = () => this.promptLiveUpdatesSequentially(rest, latest);
    if (!this.workflowActionService.checkWorkflowModificationEnabled()) {
      continueNext();
      return;
    }
    this.macroService
      .snapshotIntoInstance(node.macroId)
      .pipe(untilDestroyed(this))
      .subscribe({
        next: content => {
          try {
            // snapshotIntoInstance's response is already parsed to an object by
            // HttpClient (raw JSON body); tolerate a string too, just in case.
            const body = typeof content === "string" ? JSON.parse(content) : content;
            if (this.workflowActionService.getTexeraGraph().hasOperator(node.operatorID)) {
              const op = this.workflowActionService.getTexeraGraph().getOperator(node.operatorID);
              this.workflowActionService.setOperatorProperty(node.operatorID, {
                ...op.operatorProperties,
                snapshot: body,
                macroVersion: latestVersion,
              });
              this.notificationService.success(`Updated ${node.name} to v${latestVersion}.`);
            }
          } catch {
            /* malformed body -- leave the node on its pinned version */
          }
          continueNext();
        },
        error: () => {
          this.notificationService.error(`Failed to update ${node.name}.`);
          continueNext();
        },
      });
  }
