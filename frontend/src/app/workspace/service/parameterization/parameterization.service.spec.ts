...
describe("ParameterizationService", () => {
  ...
  describe("naming and hiding the fields inside an input", () => {
    ...
    it("says nothing until the author decides something", () => {
      ...
      expect(service.fieldOverride(service.getConfig().parameters[0], "alias")).toEqual({});
    });
    ...
    it("renames one field without touching the others", () => {
      ...
      expect(service.fieldOverride(b, "alias").displayName).toBe("Rename to");
      expect(service.fieldOverride(b, "attribute")).toEqual({});
    });

    it("hides a field and can bring it back", () => {
      service.setFieldOverride(bindingId, "alias", { hidden: true });
      expect(service.fieldOverride(service.getConfig().parameters[0], "alias").hidden).toBe(true);

      service.setFieldOverride(bindingId, "alias", { hidden: false });
      expect(service.fieldOverride(service.getConfig().parameters[0], "alias").hidden).toBeUndefined();
    });
